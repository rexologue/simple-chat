...
1. **vLLM** — OpenAI-совместимый сервер, собранный на базе CUDA контейнера с установленным `vllm` как Python-зависимостью. Модель загружается строго из локального пути `/app/model` (ничего не скачивается во время запуска).
...
- GPU с поддержкой CUDA для vLLM (пример использует `deploy.resources.reservations.devices`; подберите значение под свой рантайм). На CPU vLLM не работает. Образ собирается на базе `pytorch/pytorch:2.9.1-cuda12.8-cudnn9-runtime` и ставит `vllm` (>=0.10.0) как зависимость Python.
...
## CLI клиент для общения с gateway

В репозитории есть небольшой CLI, который общается с gateway из командной строки.

1. Скопируйте пример конфига и укажите параметры своего сервиса:
   ```bash
   cp client/chat.example.json client/chat.json
   # отредактируйте host/port и vllm_api_key
   ```
2. Запустите клиент, передав путь до конфига (по умолчанию используется `client/chat.example.json`):
   ```bash
   python client/chat.py --config client/chat.json
   ```
3. После логина скрипт предложит установить системный промпт и перейдёт в интерактивный режим. Команды:
   - `/system <текст>` — сменить системный промпт в текущей сессии;
