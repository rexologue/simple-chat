...
3. **Создайте `.env` с переменными** (пример):
   ```bash
   cat > .env <<'ENV'
   # Токен, который конечные пользователи передают в /init_session
   GATEWAY_AUTH_TOKEN=PUBLIC_SESSION_TOKEN

   # Ключ, который gateway отправляет в vLLM (должен совпадать с --api-key у vLLM)
...
| --- | --- | --- |
| `GATEWAY_AUTH_TOKEN` | Публичный токен, который передаёт клиент в `/init_session` | `CHANGE_ME` |
| `VLLM_BASE_URL` | URL vLLM внутри сети Compose | `http://vllm:8000/v1` |
| `VLLM_API_KEY` | Ключ, который gateway кладёт в `Authorization: Bearer …` при запросах к vLLM | `INTERNAL_VLLM_KEY` |
...
| --- | --- | --- |
| `VLLM_API_KEY` | Должен совпадать с тем, что знает gateway | Требуется для авторизации запросов |
...
- **Авторизация.** Единственный публичный токен `GATEWAY_AUTH_TOKEN` проверяется на этапе `/init_session`. Дальше используется только `session_id`. Между gateway и vLLM используется `VLLM_API_KEY` в заголовке Bearer.
